...
if($rot_num_rows == '1'){
	mysqli_query($conn,"UPDATE `rot` SET `label1`='$label1',`label2`='$label2',`label3`='$label3',`label4`='$label4',`personal_number`='$personal_number' WHERE `ticket_id`='$ticket_id'");
}else{
	mysqli_query($conn,"INSERT INTO `rot`(`ticket_id`, `label1`, `label2`, `label3`, `label4`, `personal_number`, `rot_data`) VALUES ('$ticket_id','$label1','$label2','$label3','$label4','$personal_number','This is static data from database only occurs on ROT enabled')");
