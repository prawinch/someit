...
if(!empty($_POST['ticket_id'])){
    ...
    $his_res=mysqli_query($conn,"SELECT * FROM `history` WHERE `ticket_id`='$ticket_id'");
    $his_row=mysqli_fetch_array($his_res);
    if(mysqli_num_rows($his_res) != '0'){
?>  
    <div class="row">
        <div class="col-lg-8">
            <div class="ibox ">
                <div class="ibox-title">
                    <h5><?php echo $ticket_id; ?></h5>
                </div>
                <div class="ibox-content">
                    <div class="activity-stream">
                    <?php
                    while($his_row=mysqli_fetch_array($his_res)){
                    ?>
                        <div class="stream">
                            <div class="stream-badge">
                                <i class="fa fa-commenting bg-warning"></i>
                            </div>
                            <div class="stream-panel">
                            <?php echo '<span class="date">'.$his_row['time'].'</span> : '.$his_row['comments'];?>
                            </div>
                        </div>
                    <?php } ?>
                    </div>
