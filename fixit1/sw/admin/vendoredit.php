...
<?php
if(!isset($_POST['edit_vendor']) AND $_GET['vendor_name'] AND $_GET['vendor_email']){
$vendor_name=$_GET['vendor_name'];
$vendor_res=mysqli_query($conn,"SELECT * FROM `vendors` WHERE `vendor_name`='$vendor_name'");
$vendor_row=mysqli_fetch_array($vendor_res);
$vendor_name=$vendor_row['vendor_name'];
$vendor_phone=$vendor_row['vendor_phone'];
$vendor_email=$vendor_row['vendor_email'];
$vendor_password=$vendor_row['vendor_password'];
?>                
                <div class="ibox float-e-margins">
                    ...
                    <form class="form-horizontal" action="vendoredit.php" method="POST" enctype = "multipart/form-data">
                                ...
                                <div class="form-group"><label class="col-lg-3 control-label">Phone</label>
                                    <div class="col-lg-4"><input type="text" class="form-control" name="vendor_phone" value="<?php echo $vendor_phone; ?>" >
                                    </div>
                                </div>
                                <div class="form-group"><label class="col-lg-3 control-label">Email</label>
                                    <div class="col-lg-4"><input type="text" class="form-control" name="vendor_email" value="<?php echo $vendor_email; ?>" >
                                    </div>
                                </div>
                                <div class="form-group"><label class="col-lg-3 control-label">Password</label>
                                    <div class="col-lg-4"><input type="text" class="form-control" name="vendor_password" value="<?php echo $vendor_password; ?>" >
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="col-sm-4 col-sm-offset-3">
                                        <button class="btn btn-white" type="reset" onclick="javascript:window.location='vendorlist.php';">Cancel</button>
                                        <button class="btn btn-primary" name="edit_vendor" type="submit">Save changes</button>
                                    </div>
                                </div>
                            </form>                        
...
<?php
}else if(isset($_POST['edit_vendor'])){
        $vendor_name=$_POST['vendor_name'];
        $vendor_phone=$_POST['vendor_phone'];
        $vendor_email=$_POST['vendor_email'];
        $vendor_password=$_POST['vendor_password'];
        $result=mysqli_query($conn,"SELECT * FROM `vendors` WHERE `vendor_name`='$vendor_name'");
        $rows1=mysqli_num_rows($result);
        if($rows1==1){
          mysqli_query($conn,"UPDATE `vendors` SET `vendor_name`='$vendor_name',`vendor_email`='$vendor_email',`vendor_phone`='$vendor_phone',`vendor_password`='$vendor_password' WHERE `vendor_name`='$vendor_name'");
?>
