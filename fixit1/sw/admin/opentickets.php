...
<body>
    <div id="wrapper">
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            ...
            <a href='#Close' data-toggle='modal' data-hover='tooltip' title='Close Ticket' data-placement='top' data-whatever='".$row[ 'ticket_id']. "' class='btn btn-warning btn-circle btn-outline'><span class='glyphicon glyphicon-remove'></a>
            ...
            <a href='#Reminder' data-toggle='modal' data-hover='tooltip' title='Send Reminder' data-placement='top' data-whatever='".$row[ 'ticket_id']. "' class='btn btn-primary btn-circle btn-outline'><span class='glyphicon glyphicon-send'></a>
              </td></tr>";
            }
            ?>
                    </tbody>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="CloseLabel">Close the Ticket</h4>
            </div>
            <form id="closeformdata" method="POST">
...
$("#Close").on("show.bs.modal",function(e){var a=$(e.relatedTarget),t=a.data("whatever"),o=$(this);o.find(".modal-body input").val(t)}),$(function(){$("#subscribe-email-form").on("submit",function(e){e.preventDefault(),$.ajax({url:"opentickets.php",type:"GET",data:$("#subscribe-email-form").serialize(),success:function(e){$("#Close").modal("toggle")}})})});
</script>
<?php
if (isset($_POST[ 'ticketclose']) AND isset($_POST[ 'ticket_id'])){
  ...
  $ini_email=$email_row['ini_email'];
          ...
          $message .= 'Your Ticket has been closed by admin!<br><br> Your unique tracking code is '.$ticket_id.'<br><br>';
...
?>
          <script type="text/javascript">window.document.location = "closedtickets.php"</script>
<?php
...
if (isset($_GET[ 'ticketdelete']) AND isset($_GET[ 'ticket_id'])){
  ...
  $ini_email=$mail_row['ini_email'];
          ...
          //phpmailer starts here
          $url='http://'. $_SERVER['SERVER_NAME'].'/fixit1';
          $message = '<html><head><title>Ticket</title></head><body>';
          $message .= '<h4>Hi ' . $ini_name . '!</h4></br>';
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="DeleteLabel">Send Reminder for this Ticket</h4>
            </div>
            <form id="deleteformdata">
                <div class="modal-body">
                    <input type="hidden" name="ticket_id" class="form-control" id="recipient-name">
...
if (isset($_GET[ 'ticketreminder']) AND isset($_GET[ 'ticket_id'])){
  ...
  //phpmailer starts here
          ...
          $message = '<html><head><title>Ticket</title></head><body>';
          $message .= '<h4>Hi ' . $ini_name . '!</h4></br>';
          $message .= 'Admin sent a Reminder for your ticket - '.$ticket_id.'<br><br>';
          $message .= 'Phone number associated with this ticket is '.$ini_phone.'<br><br>';
          $message .= 'Track your ticket at <a href="'.$url.'/track.php?ticket_id='.$ticket_id.'&phone='.$ini_phone.'">'.$url.'/track.php?ticket_id='.$ticket_id.'&phone='.$ini_phone.'</a><br><br>';
          $message .= 'Regards,<br>Fixit<br><br>For any queries please contact us at +46-9999999<br></body></html>';
